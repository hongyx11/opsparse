#include "opsparse/numeric/compute_flop.hpp"

#include <omp.h>

#include "opsparse/core_ds/csr.hpp"

namespace opsparse {

__global__ void __launch_bounds__(1024, 2)
    k_compute_flop(const mint *__restrict__ d_arpt, const mint *__restrict__ d_acol, const mint *__restrict__ d_brpt, mint M,
                   mint *d_row_flop, mint *d_max_row_flop)
{
    __shared__ mint shared_max_row_flop[1];
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= M) return;
    if (threadIdx.x == 0) shared_max_row_flop[0] = 0;
    __syncthreads();
    mint row_flop = 0;
    const mint arow_start = d_arpt[i];
    const mint arow_end = d_arpt[i + 1];
    for (mint j = arow_start; j < arow_end; j++) {
        const mint acol = d_acol[j];
        row_flop += d_brpt[acol + 1] - d_brpt[acol];
    }
    d_row_flop[i] = row_flop;
    atomicMax(shared_max_row_flop, row_flop);
    __syncthreads();
    if (threadIdx.x == 0) {
        atomicMax(d_max_row_flop, shared_max_row_flop[0]);
    }
}

long compute_flop(const mint *row_ptr_a, const mint *col_idx_a, const mint *row_ptr_b, mint M, mint *row_flop_out)
{
    long total_flop = 0;
#pragma omp parallel
    {
        long thread_flop = 0;
#pragma omp for
        for (mint i = 0; i < M; i++) {
            long local_sum = 0;
            for (mint j = row_ptr_a[i]; j < row_ptr_a[i + 1]; j++) {
                local_sum += row_ptr_b[col_idx_a[j] + 1] - row_ptr_b[col_idx_a[j]];
            }
            row_flop_out[i] = local_sum;
            thread_flop += local_sum;
        }
#pragma omp critical
        { total_flop += thread_flop; }
    }
    return total_flop;
}

long compute_flop(const CSR &A, const CSR &B, mint *row_flop)
{
    return compute_flop(A.rpt, A.col, B.rpt, A.M, row_flop);
}

long compute_flop(const CSR &A, const CSR &B)
{
    mint *row_flop = new mint[A.M];
    long flop = compute_flop(A.rpt, A.col, B.rpt, A.M, row_flop);
    delete[] row_flop;
    return flop;
}

}  // namespace opsparse
